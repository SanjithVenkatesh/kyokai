...
class Torikumi:
    ...
    def processMatches(self, year: int, month: int, day: int) -> Dict[str, Bout]:
        ...
        torikumiLines = [i.rstrip("\r") for i in torikumiLines if len(i) > 0 and i != '\r']
            ...
            if v in constants.DIVISIONS:
                ...
                if len(splitValue) > 0:
                    ...
                    if len(kimariteInter) > 0:
                        winner, kimarite, loser = splitLine[0:3], splitLine[3], splitLine[4:]
                        matches[currentDivision].append(Bout(winner[1], loser[1], kimarite))
                    else:
