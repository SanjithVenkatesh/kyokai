...
class Banzuke:
            ...
            if len(rInfo) > 1:
                try:
                    if len(rInfo) == 6 and len(re.findall(r'[A-Z]',rInfo[2])) > 1:
                        ...
                        rikDict[currentDiv].append(Rikishi(rInfo[1], splitOriginBeya[0], splitOriginBeya[1], rInfo[3], rInfo[4], rInfo[5], rInfo[0]))
                    else:
                        rikDict[currentDiv].append(Rikishi(rInfo[1], rInfo[2], rInfo[3], rInfo[4], rInfo[5], rInfo[6], rInfo[0]))
    ...
    def getBanzuke(self, year, month):
        ...
        banzukeLines = [i.rstrip('\r') for i in banzukeLines if i is not None and len(i) > 0 and i != '\r']
    ...
    def getRikishiByBeya(self, beya) -> List[Rikishi]:
        return [rikishi for division in self.rikishis.values() for rikishi in division if rikishi.beya == beya]